...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  let {blog, blog : {author :{ personal_info : { username: blog_author } } ,comments,activity, activity:{ total_parent_comments} , comments : {results : commentsArr}}, setBlog,setTotalParentCommentsLoaded} = useContext(blogContext)
  ...
  const deleteComment = (e) =>{
    ...
    axios.post(import.meta.env.VITE_DOMAIN_SERVER + "/delete-comment", {_id },{
        headers : {
  ...
  const handleReplyClick = () =>{
    if(!access_token){
        return toast.error("You need an account to leave a reply!")
    }
  ...
  const loadReplies =( { skip = 0 })=>{
    ...
    if(children.length){
        ...
        axios.post(import.meta.env.VITE_DOMAIN_SERVER + "/get-replies", { _id, skip })
        .then( ( { data:{replies}}) =>{
