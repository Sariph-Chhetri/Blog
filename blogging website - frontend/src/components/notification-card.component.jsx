...
const NotificationCard = ({data, index, notificationState}) => {
    ...
    const handleDelete = ( comment_id, type, target) =>{
        ...
        axios.post(import.meta.env.VITE_DOMAIN_SERVER + "/delete-comment", { _id: comment_id }, {
            headers :{
        ...
        .then( () =>{
            ...
            setNotifications({ ...notifications, results, totalDocs: totalDocs - 1, deleteDocCount : notifications.deleteDocCount })
        })
        .catch(err =>{
            console.log(err);
        })
  ...
  return (
    <div className={'p-6 border-b border-grey border-l-black ' + (!seen ? "border-l-2" : "" )}>
        ...
        <div className='flex gap-5 mb-3 '>
            <img className='w-14 h-14 flex-none rounded-full' src={profile_img} alt='error'/>
...

        {
            ...
            <div className='ml-20 p-5 bg-grey mt-5 rounded-md'>
               <div className='flex gap-3 mb-3'>
                <img className='w-8 h-8 rounded-full' src={author_profile_img} alt='error' />
                <div>
                    <h1 className='font-medium text-xl text-dark-grey'>
               ...
               <p className='ml-14 font-gelasio text-xl my-2'> {reply.
               comment} </p>

               <button className='underline text-dark-grey hover:text-black ml-14 mt-2' onClick={(e)=>{handleDelete(reply._id,"comment",e.target)}}>Delete</button>
