...
const NotificationCommentField = ( { _id, blog_author, index = undefined, replyingTo = undefined, setReplying, notification_id, notificationData } ) => {
    ...
    const handleComment = () =>{
            ...
            if(!comment.length){
                toast.error("Write something to leave a reply");
            }
    
            axios.post(import.meta.env.VITE_DOMAIN_SERVER + "/add-comment",{ _id, blog_author:user_id, comment, replying_to: replyingTo, notification_id },{
                headers :{
